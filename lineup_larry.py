...
def lineup_larry(folder):
    ...
    input_files = [f for f in os.listdir(folder) if os.path.splitext(f)[1].lower() == '.png']
    if not input_files:
    ...
    output_image = Image.new('RGB', (num_columns * max_width, num_rows * max_height), 'white')
    ...
    output_file_name = os.path.join(folder, folder_name + '.png')  # Create the output file path
...
def draw_j(image, x_offset, y_offset, img_width, img_height):
    ...
    draw.text((x_offset + img_width - 30, y_offset + img_height - 30), text, fill='black', font=font)
